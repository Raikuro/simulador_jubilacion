...
- Whenever a governance or continuity document is modified, any local duplication discovered within that document should be removed as part of the same change, provided no architectural meaning is altered.
...
### Commit Governance
Implementation milestone work is governed by the project's atomic commit policy. Implementation Engineers must not create intermediate commits during the execution of a single milestone. The final commit may only be created after the milestone is fully implemented, validated, self-reviewed, audited, and accepted.
